...
module.exports = async function (input, options = {}) {
  ...
  if (Discord.version.split(".")[0] < 14)
    return console.log(
      "Discord.js Akinator Error: Discord.js v14 or Higher is Required.\nNeed Help? Join Our Discord Server at 'https://discord.gg/P2g24jp'"
    );
  ...
  try {
    ...
    if (!fs.existsSync(`${__dirname}/translations/${options.language}.json`))
      return console.log(
        `Discord.js Akinator Error: Language "${options.language}" Not Found. Examples are: "en" or "fr" or "es".\nNeed Help? Join Our Discord Server at 'https://discord.gg/P2g24jp'`
      );
    ...
    if (input.commandName !== undefined && !input.replied && !input.deferred) {
    ...
    if (input.commandName !== undefined) {
      ...
      akiMessage = await startingMessage.edit({ embeds: [akiEmbed] });
    } // else, the input is a message
    ...
    while (notFinished) {
      ...
      if (
        ...
        let guessEmbed = {
          title: `${await translate(
            `I'm ${Math.round(aki.progress)}% sure your ${
              options.gameType
            } is...`,
            options.language
          )}`,
          description: `**${aki.answers[0].name}**\n${await translate(
            ...
            options.gameType == "animal"
              ? translations.isThisYourAnimal
              : options.gameType == "character"
              ? translations.isThisYourCharacter
              : translations.isThisYourObject
          } ${
            !options.useButtons
              ? `**(Type Y/${translations.yes} or N/${translations.no})**`
              : ""
          }`,
          color: options.embedColor,
          ...
          fields: [
            {
              name: translations.ranking,
              value: `**#${aki.answers[0].ranking}**`,
              inline: true,
            },
            {
              name: translations.noOfQuestions,
              value: `**${aki.currentStep}**`,
              inline: true,
            },
          ],
        };
          ...
          .then(async (response) => {
            ...
            if (
              ...
              let finishedGameCorrect = {
                ...
                fields: [
                  {
                    name: translations[options.gameType],
      ...
      if (updatedAkiEmbed !== akiMessage.embeds[0]) {
        updatedAkiEmbed = {
          title: `${translations.question} ${aki.currentStep + 1}`,
          description: `**${translations.progress}: ${Math.round(
            aki.progress
      ...
      await awaitInput(
        ...
        .then(async (response) => {
          ...
          if (options.useButtons)
            ...
            await akiMessage.edit({ embeds: [thinkingEmbed], components: [] });
    ...
    console.log("Discord.js Akinator Error (ignore this)");
  }
};
