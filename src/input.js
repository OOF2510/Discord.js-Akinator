...
module.exports = async function awaitInput(
  ...
  if (useButtons) {
    ...
    let probablyNot = {
      type: 2,
      label: translations.probablyNot,
      style: 2,
      custom_id: "👎",
      emoji: { name: "👎" },
    };
    ...
    if (!response.size) {
      ...
      if (["y", "n", "i", "idk", "p", "pn", "b", "s"].includes(responseText))
        return responseText; //skip translation for these responses
